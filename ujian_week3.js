...
function naikAngkot(arrPenumpang) {
  ...
  arrPenumpang.forEach((penumpang) => {
    const [namaPenumpang, dari, sampai] = penumpang;
    const indexDari = rute.indexOf(dari);
    const indexSampai = rute.indexOf(sampai);
    const harga = (indexSampai - indexDari) * 2000;
    hasil.push({
...
function highestScore(students) {
  ...
  students.forEach((student) => {
    ...
    if (!result[studentClass] || result[studentClass].score < studentScore) {
      result[studentClass] = {
        name: student.name,
        score: studentScore,
      };
...
function graduates(students) {
  ...
  students.forEach((student) => {
    if (student.score > 75) {
      if (!yeyyLulus[student.class]) yeyyLulus[student.class] = [];

      yeyyLulus[student.class].push({
        name: student.name,
        score: student.score,
      });
    }
