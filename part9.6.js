// soal 1
function changeMe(firstArrays) {
  if (firstArrays.length === 0) {
    console.log('""');
    return;
  }

  firstArrays.forEach((firstArray) => {
    let [firstName, lastName, gender, birthYear] = firstArray;

    let currentYear = new Date().getFullYear();
    let age = birthYear ? currentYear - birthYear : "Invalid Birth Year";
...
function countProfit(shoppers) {
  ...
  for (const list of listBarang) {
    let [product, price, stock] = list;
    ...
    for (const shopper of shoppers) {
      if (
        shopper.product === product &&
        shopper.amount <= productResult.leftOver
      ) {
        productResult.shoppers.push(shopper.name);
        productResult.leftOver -= shopper.amount;
        productResult.totalProfit += shopper.amount * price;
      }
    }
